...
export const ECON_ESSAY_ANSWERS_3 = {
  ...
  Q069: A(
    ...
    [
      [
        ...
        'How long this takes depends heavily on the specific industry: a firm with under-used capacity or a warehouse of stock can raise output within weeks by running an extra shift or releasing stock, while a firm already at full capacity in a sector with long lead times, such as building a new semiconductor fabrication plant, may take several years to expand - which is why the global chip shortage of 2021 held up car production for well over a year despite demand being obvious to everyone involved. “Long run” is therefore a different amount of calendar time in different industries, not a fixed period.',
      ],
      [
  ...
  Q073: A(
    ...
    [
      [
        ...
        'Because the firm never has to pay for the external cost it creates, it has no incentive to account for it, and it produces up to the point where its own MPC equals demand - a quantity that is higher than the socially optimal output where MSC would equal demand. The good is therefore overproduced relative to the level that would maximise society’s total welfare, and the units produced beyond the social optimum create a welfare loss because their true cost to society exceeds the benefit consumers gain from them. Coal-fired power generation is the classic case: the generator pays for the coal, the plant and the labour, but not for the health damage the emissions cause to people living downwind, so it burns more coal than the socially optimal amount.',
      ],
      [
  ...
  Q088: A(
    ...
    [
      [
        'With only a few large firms, price is typically set above marginal cost, since each firm has some market power rather than facing the perfectly elastic demand of a price-taker, so allocative efficiency in the strict, static sense is not achieved: the price paid exceeds the true marginal cost of the last unit produced, and output is lower than the allocatively efficient level. Nor does a small number of firms guarantee production at the lowest point of the average cost curve: without the constant pressure of many rivals and free entry, X-inefficiency - costs drifting above the minimum because there is less pressure to cut them - can persist, so productive efficiency is at risk as well.',
        'How large either loss is depends on how the few firms behave. The gap between price and marginal cost is smallest where they compete hard or the market is contestable, since the threat of entry disciplines price without any entry taking place, and largest where they collude; and X-inefficiency is a risk rather than a certainty, since large firms under pressure from shareholders and from each other can be lean. The static losses are therefore an upper bound that depends on rivalry, not a fixed cost of the structure.',
      ],
      [
  ...
  Q089: A(
    ...
    [
      [
        ...
        'As new firms enter, total market supply increases, shifting the market supply curve to the right and driving the market price down, while each individual firm’s market share falls. This process continues, with more firms entering and price falling further, until price has fallen to the point where it exactly equals each firm’s average cost at its profit-maximising output - the point of zero economic profit - at which point there is no longer any incentive for further entry, and the market settles into long-run equilibrium. Agriculture behaves this way in practice: a season of high prices for a crop draws more growers into planting it, and the resulting supply pushes the price back down to a level that covers costs and little more.',
      ],
      [
  ...
  Q093: A(
    ...
    [
      ...
      [
        ...
        'How much this matters depends on how much genuine monopsony power actually exists. A single large employer in an isolated local labour market - a hospital trust for a specific specialism in a town with no comparable alternative employer - has real wage-setting power of this kind. The NHS is the standard British illustration, since it employs the overwhelming majority of nurses in the country and sets their pay through national pay bands rather than by competing against rival employers for each hire. In a national, highly mobile occupation with many competing employers, however, workers can simply move to whichever firm offers the best wage, which erodes any one employer’s monopsony power almost entirely.',
      ],
    ],
    null,
    [
  ...
  Q115: A(
    ...
    [
      [
        'The first characteristic is product differentiation: each firm’s product is a close, but not perfect, substitute for its rivals’ - through branding, features, quality or location - which gives the firm a downward-sloping demand curve (AR = D) rather than perfect competition’s horizontal one, since it can raise its price a little without losing every customer, some of whom prefer its particular version of the product. The sandwich and coffee trade on any British high street - Pret, Greggs and a dozen independents within a few hundred metres - is the picture: each has its own regulars, and none can raise its price far.',
  ...
  Q131: A(
    ...
    [
      [
        'The first is a change in real income. For a NORMAL good, higher real income means consumers can afford more at every price, so demand shifts right from D₁ to D₂ and, with supply unchanged, equilibrium price rises to P₂ and quantity to Q₂. Rising real wages increase demand for restaurant meals, new cars and foreign holidays in exactly this way, and the effect is strongest for luxuries, whose income elasticity exceeds 1. When UK real wages fell in 2022-23 the same shift ran in reverse, with spending on eating out and holidays falling first - the normal-good signature seen from the other side.',
  ...
  Q137: A(
    ...
    [
      [
        'Capture happens because of an asymmetry in information and incentives. The regulator depends on the industry for the data it needs to regulate - costs, capacity, technical constraints - and the firms have every reason to present that information in the light most favourable to themselves. The Financial Services Authority before 2008 is the standard British case: staffed and informed largely by the industry it regulated, it accepted the banks’ own assessments of their risk until those assessments failed. Meanwhile the benefit of a lenient decision is large and concentrated for a handful of firms, who will lobby hard for it, while the cost is small and spread across millions of consumers, none of whom has enough at stake individually to organise against it. The regulator hears sustained, well-resourced argument from one side and almost nothing from the other.',
  ...
  Q141: A(
    ...
    [
      ...
      [
        'The higher quantity Q₂ is usually the real objective rather than the price change itself. Where the good generates positive externalities - vaccination, public transport, insulation - the free market under-consumes it because private buyers ignore the benefit to third parties, and the subsidy raises consumption towards the socially optimal level by making the private cost better reflect the social benefit. The £2 bus fare cap in England from 2023 is a subsidy of exactly this shape, paid to operators so that the fare passengers pay falls and passenger numbers rise.',
