...
export const ECON_ESSAY_ANSWERS_AS = {
  ...
  Q004: A(
    ...
    [
      [
        'Tobacco duty is a SPECIFIC tax - a fixed amount per unit, which shifts supply up by the same cash amount at every price - rather than an AD VALOREM tax such as VAT, which is charged as a percentage of price and so pivots the supply curve, widening the gap as price rises. A specific duty on cigarettes shifts the supply curve vertically upwards by the amount of the tax. The price rises and quantity demanded contracts, moving consumption from Q1 towards Q*. If the tax is set equal to the external cost at the optimum, smokers pay the full social cost of their choice and the externality is internalised. The tax also raises substantial revenue, which can be hypothecated to healthcare and anti-smoking campaigns, so it both reduces the externality and funds the treatment of its consequences. Compared with a ban, it preserves consumer choice for those who value smoking highly.',
